...
PROMPT="%{%(!.$fg_bold[red].$fg_bold[green])%}%n@%m%{$reset_color%} %{$fg_bold[blue]%}%~%{$reset_color%} %{$fg_bold[white]%}%(!.☭.⚘) "
...
function title {
    ...
    if [[ $USER != 'wayne' && $USER != 'wayne2' ]]; then
        prefix="[$USER] "
    fi
    ...
    if [[ $prefix != '' && $USER != 'wayne' && $USER != 'amunroe' ]]; then
        prefix="$USER@$prefix"
    fi
    # Wrap it in brackets
    if [[ $prefix != '' ]]; then
...
if [ -f /usr/local/bin/virtualenvwrapper.sh ]; then
    ...
    echo "Virtualenvwrapper not installed in the normal place"
fi
